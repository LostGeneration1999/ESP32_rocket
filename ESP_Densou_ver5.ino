...
//SDfileを設定
const char* fname = "/ESP_Densou_version4_0.txt";
...
// 書き込みモードでファイルを開く
  ...
  if(fo){
    ...
    fo.println(String("Latitude  : ")+gps.location.lat());
    fo.println(String("Longitude  : ")+gps.location.lng());
    fo.println(String("Satellites: ")+gps.satellites.value());
    fo.println(String("Altitude  : ")+gps.altitude.feet() / 3.2808);
    fo.print(String("Time      : ")+gps.time.hour()+9);
    fo.print(String(":")+gps.time.minute());
    fo.println(String(":")+gps.time.second());
//    fo.println(String("Reason:")+reason);
    fo.println(String("**********************"));
    fo.close();
  }
  else{
